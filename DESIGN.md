...
当前已用 `Option<CaptureOperation>` 把捕获尝试、截图会话、冻结图、编辑状态和截图窗口资源从 `App` 收进同一个生命周期边界，并以 `Preparing`、`Selecting`、`Editing` 表示阶段。`handler.rs` 仍是 winit 事件适配器，迁移期内部分事件会通过窄转发访问会话状态；平台调用也没有完全收口：捕获编排仍使用 xcap、winit 和 softbuffer，剪贴板直接调用 Win32，OCR 编排直接启动子进程并在回退路径调用 WinRT OCR。
...
| 截图操作与会话状态 | 捕获尝试和截图会话生命周期、阶段、冻结图、选区、编辑状态、Window/Surface 释放顺序 | `src/app/capture_operation.rs` |
...
本节同时记录已确认目标和迁移边界。当前已落地截图操作所有权、显式阶段、纯状态迁移测试，以及“先完整创建新贴图，再结束旧会话”的交接顺序；原始 `WindowEvent` 的完整收口和轻量画面描述仍是后续迁移项。
...
`capture_operation` 是对外的深模块。它内部拆分纯状态和窗口资源：

- 纯状态拥有冻结图、选区、标注和阶段，是截图操作的唯一状态真相；
- 纯状态根据领域事件产生高层动作，不包含 winit、Win32 或窗口资源；
- 窗口资源执行窗口创建、显隐、呈现和释放，返回稳定失败类别及技术详情；
- 外层接收属于截图窗口的原始 `WindowEvent`，转换成领域事件，协调动作执行，并生成用户提示；
...
| 状态结构 | 截图操作所有权边界 + 可测试状态迁移 | 会话资源归属明确，关键转移可独立测试 | 事件收口仍需渐进迁移 | **当前选择** |
...
| D-11 | 按截图操作、编辑、几何、渲染、剪贴板、OCR、贴图和 Windows 边界拆分；`App/handler` 保留平台事件与动作编排 | 限制变化扩散，并把单次截图的数据和资源归入同一所有权边界 | 事件适配和平台调用尚未完全收口 |
...
| D-14 | 将捕获尝试和截图会话收拢为截图操作深模块；内部以纯状态和窗口资源两个模块协作，阶段数据用枚举隔离；`App` 只持有可选截图操作并路由外部事件 | 集中会话不变量、失败恢复和动作顺序，使 interface 成为状态迁移测试表面，并为后续异步 OCR 降低状态组合风险 | 迁移期间存在临时转发；阶段切换需要移动所有权；窗口烟测仍依赖 Windows/winit |
