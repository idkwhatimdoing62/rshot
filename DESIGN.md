...
> 产品版本：v0.2.14
>
> 代码基线：本地 `main` 的 `71d8dd0` 加本次未提交的高精度 OCR 工作树改动；提交后以对应提交哈希替换本行
>
...
当前没有为 OCR 延迟设定跨设备达标数值。已记录一组 42 行 1080p 合成页基线，但主进程仍会同步等待一次性 worker，单次总超时为 20 秒且当前不能手动取消。还需要在其他 CPU 上重复 1080p，并测量 4K、近全屏和超大图缩放场景，再决定是否改为异步返回并支持用户取消。
