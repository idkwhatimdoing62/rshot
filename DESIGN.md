...
> 文档状态：当前实现基线
>
> 代码基线：当前 `main`（基于 v0.2.14，包含其后的零面积选区修复）
>
...
rshot 在一个冻结画面上完成选择、编辑、OCR、复制和置顶贴图。没有贴图时，程序待命不持有活动截图像素、遮罩窗口或标注内容；运行时仍保留托盘、热键、配置、事件状态和可复用容器容量。保留贴图时，每张贴图额外持有最终像素与窗口资源。

### 1.3 成功条件

- 用户只能通过全局截图热键进入截图会话；
- 双击托盘图标显示作者、版本、项目地址、当前热键和诊断日志设置，不触发截图；
...
| 托盘 | 双击托盘图标 | 显示作者、版本、项目地址、当前截图/退出热键及诊断日志状态；不得创建截图会话 |
| 鼠标 | 位置、左键、右键、拖动 | 含义由主状态和命中区域决定 |
| 键盘 | 工具快捷键、动作快捷键、文字输入 | 文字输入状态优先消费字符，避免把 `O`、`P` 等字符误当动作 |
| 输入法 | 预编辑文本、提交文本、启用和停用事件 | 只在文字草稿状态处理 |
| Windows 窗口系统 | 显示器、顶层窗口边界、DPI、剪贴板、OCR 语言包 | 由 Windows 辅助、剪贴板、OCR 模块及捕获编排分别转换和处理 |
| 配置文件 | `hotkey`、`quit`、`diagnostics` | 启动时读取；注册两个热键并设置捕获失败日志开关，修改后重启生效 |

### 2.2 输出与状态变化

| 输出 | 内容 | 成功后的状态 |
| --- | --- | --- |
| 图片剪贴板 | 同一次发布尝试写入 `CF_DIB` 位图，以及通过 `CF_HDROP` 提供 `%TEMP%\rshot-clipboard\` 中的唯一 PNG | 至少一种格式发布成功且剪贴板正常关闭后回到待命，否则保留当前会话 |
| 文字剪贴板 | OCR 结果，使用 `CF_UNICODETEXT` | 回到待命 |
| 置顶窗口 | 已裁剪并合成标注的图像 | 新增一张独立贴图，截图会话回到待命 |
| 错误提示 | 启动、捕获入口、会话渲染、单张贴图、图片剪贴板或 OCR 错误 | 启动错误退出；捕获与会话渲染错误回到待命；贴图错误只关闭故障贴图；剪贴板与 OCR 错误保留当前编辑会话 |
...
默认值为 `Alt+A`、`Alt+D` 和开启捕获失败诊断。旧配置缺少 `diagnostics` 时自动使用默认值；配置只在启动时加载，运行中不热更新。

### 3.2 截图会话

```mermaid
classDiagram
    class AppSessionFields {
        +Boolean capture_attempt_active
        +Option~Window~ window
        +Option~Surface~ surface
        +Option~RgbaImage~ img
        +Point origin
        +RectI[] windows
    ...
    AppSessionFields *-- EditorState
```

这是一张 `App` 中会话相关字段的概念视图，并不存在名为 `ScreenshotSession` 的独立结构体。窗口、Surface、原图和选区在待命或捕获尝试的中间阶段都可以为空；只有正式活动会话要求窗口、Surface 和原图同时存在。

- `img` 在选择态和编辑态保存未经标注的原始冻结图；
- `capture_attempt_active` 从接受截图热键起保持为真，窗口和 Surface 成功提交或失败清理后归零，用于阻止入口重入和同一次失败重复提示；正式会话继续由 Window、Surface 和原图资源表示；
- `origin` 负责屏幕坐标与遮罩窗口坐标之间的转换；
- `windows` 在遮罩出现前按 Z 序保存，自动锁定取第一个命中的窗口；
- `sel` 保存两个对角点，使用时再归一化和裁剪；
- `editor.annotations` 使用遮罩窗口内的绝对坐标，输出时减去选区左上角偏移。
...
classDiagram
    ...
    Annotation --> Shape : 具体形状
    Shape --> PenData : Pen 载荷
    Shape --> LineData : Line 载荷
    Shape --> RectData : Rect 载荷
    Shape --> TextData : Text 载荷
```
...
classDiagram
    ...
    note for PinnedWindow "drag 保存全局光标起点和按下时查询的窗口起点；max(图像尺寸, 56 × 44) 只是创建时请求的窗口尺寸"
```

App 使用 `HashMap<WindowId, PinnedWindow>` 保存贴图，数量上限为 8。关闭、拖动和渲染事件只作用于命中的窗口 ID。

创建贴图时至少请求 56×44 的窗口尺寸，为控制入口保留区域，但当前没有持续施加系统级最小尺寸约束。图像小于窗口时，空余区域不改变原图尺寸；渲染器用黑色填充，因此极小贴图可能出现黑色边带。左上角蓝色 `PIN` 标识和右上角红色 `X` 会覆盖窗口像素；这些均属于贴图窗口控件，不属于原图内容。

### 3.5 OCR 输入

```mermaid
flowchart LR
    A["img<br/>原始冻结图"] --> B["按 sel 裁剪"]
    B --> C{"超过 OCR<br/>最大尺寸？"}
    C -- 是 --> D["等比缩小"]
    C -- 否 --> E["保持原尺寸"]
    D --> F["OcrInput"]
...
13. 两个显示器后端都匹配成功后、屏幕捕获前，清除贴图拖动状态、隐藏全部贴图并刷新 DWM；此前的入口失败不改变贴图可见性或拖动状态，一旦进入隐藏阶段，结束或失败后必须无激活地恢复；
14. 关闭活动截图窗口后，下一次截图必须从新的屏幕捕获和空标注集合开始；
15. 屏幕坐标与当前显示器窗口坐标只通过 `origin` 换算；
...
`Mode` 只表示活动截图窗口内部的选择或编辑状态。待命状态由活动截图 `window = None` 表示；`Capturing` 是 `capture_attempt_active = true` 的瞬时入口阶段，不属于 `Mode`。贴图集合与这条状态机正交存在。
...
stateDiagram-v2
    ...
    Selecting --> Editing: 单击锁定窗口 / 有面积拖框松手
    Editing --> Selecting: R / SELECT
    Selecting --> Standby: Esc / 复制成功 / 置顶成功 / 会话故障
    Editing --> Standby: X / Esc 落到最终优先级 / 复制成功 / 置顶成功 / OCR 与文字剪贴板成功 / 会话故障
    Selecting --> Selecting: 复制失败 / 贴图达到上限
    Editing --> Editing: 复制失败 / 贴图达到上限 / OCR 为空或失败
```

双击托盘只显示信息，不改变状态；`Alt+D` 可在任意阶段退出程序。编辑态的 `Esc` 只有在没有文字草稿和未打开色板时才结束会话，具体优先级见第 4.4 节。

任意主状态下可以同时存在 0～8 张贴图。贴图的拖动、关闭和重绘按自身 `WindowId` 路由，不改变活动截图状态；捕获尝试匹配两个显示器后才暂时隐藏全部贴图，一旦隐藏，截图会话结束或失败时都会恢复。
...
| 捕获尝试 `Capturing`（不属于 `Mode`） | 捕获前的桌面；进入隐藏阶段后贴图暂时不可见 | 无 | 失败时清理并回到待命；成功后进入选择态 |
| 选择 `Selecting` | 冻结画面、遮罩和红色选框 | 单击锁定窗口；拖动建立手动选区 | 复制或置顶成功、取消、退出 |
| 编辑 `Editing` | 选区、标注和工具栏 | 在工具栏操作，或在选区内创建标注 | 复制/OCR/置顶成功、重新选择、取消、退出 |
| 独立贴图（不属于 `Mode`） | 无边框置顶图、左上角 `PIN` 标识和右上角关闭按钮 | 拖动当前贴图；命中关闭按钮时只关闭当前贴图 | 右键、`Esc`、关闭按钮、退出 |

复制失败、OCR 为空或失败、以及贴图达到上限都不会结束当前选择或编辑状态。
...
| 单击松手且没有任何选区 | 保持选择态 |
| 有面积的拖框松手 | 归一化后宽、高均大于零时，固定手动选区并进入编辑态 |
| 纯水平或纯垂直拖动松手 | 丢弃零面积手动框，保持选择态，并立即恢复光标下窗口的自动锁定；未命中窗口时选区为空 |
...
1. 清理任何不完整的旧会话，建立新的捕获尝试；
2. 读取鼠标位置；
3. 分别定位 xcap 使用的捕获显示器和 winit 使用的遮罩显示器；
4. 保存遮罩显示器原点；
5. 两个显示器都匹配成功后，结束贴图拖动、无激活地隐藏全部贴图并刷新 DWM；
6. 捕获整屏 RGBA 图像；
7. 保存原图，并重置选区、标注、工具、颜色和输入状态；
8. 在遮罩出现前记录可锁定窗口；
9. 创建全屏无边框 Window、图形 Context 和 Surface，启用 IME 并请求首帧；
10. 提交 Window 与 Surface，结束捕获尝试。

**成功结果**：进入选择态，默认工具为红色画笔。

**失败结果**：鼠标读取或两个显示器定位失败发生在隐藏贴图之前，不改变贴图可见性；屏幕捕获或窗口资源创建失败时恢复已经隐藏的贴图。所有失败都会清理不完整会话并保持待命。
...
贴图存在时仍可使用截图热键。两个显示器后端匹配成功后、屏幕捕获前，全部贴图暂时隐藏并刷新 DWM，避免进入截图或盖住遮罩；复制、取消、后续失败或生成新贴图后无激活地恢复。双击托盘仍只显示信息窗口。
...
当前实现已经按能力拆成多个文件，但还没有独立的“内部事件 → 纯状态机 → 动作”层。`App` 仍持有完整可变会话状态和窗口资源，`handler.rs` 直接接收系统事件并修改 `App`；平台调用按能力分布在 Windows 辅助、剪贴板和 OCR 模块中：
...
| `src/app/diagnostics.rs` | 捕获失败诊断行、配置文件同目录日志和 64 KiB 大小上限 |
| `src/app/pinned.rs` | 独立贴图窗口、拖动状态、绘图表面、重绘和最多八张限制 |
| `src/app/state.rs` | 主模式枚举、捕获失败阶段和可恢复会话错误；选区与窗口生命周期仍由应用会话持有 |
| `src/app/geometry.rs` | 矩形归一化、选区面积判断、命中判断、裁剪和平台无关窗口矩形 |
...
| 捕获诊断 | 稳定错误码、允许记录的固定字段和日志上限 | 捕获失败阶段、系统时间 | 一行无隐私诊断或可忽略的写入失败 |
| Windows 通用辅助 | DPI、窗口枚举、DWM、GDI、消息框和 WinRT 生命周期 | 平台调用参数 | 内部坐标、像素、文本或平台错误 |
| 截图捕获编排 | xcap/winit 显示器匹配、RGBA 原图和窗口资源创建 | 鼠标屏幕坐标和事件循环 | 原图、显示器原点、活动会话或捕获失败 |
| 会话状态与事件编排 | 主状态、选区、草稿、优先级、窗口资源和状态转移 | winit、热键与托盘事件 | 直接修改 `App` 并调用对应操作 |
...
- 当前没有独立纯状态机；`handler.rs` 直接把 winit、热键和托盘事件路由到 `App`，后续若抽取状态机必须保持本文档的状态转移；
- Win32/WinRT 调用集中在 `windows_adapter.rs`、`clipboard.rs` 和 `ocr.rs`，xcap 捕获由 `mod.rs` 直接编排；几何与大部分编辑逻辑保持平台无关；
- 编辑器只修改标注数据，不负责剪贴板和窗口生命周期；
- OCR 接口接收原图和选区，调用方不能传入已合成标注的图片；
- 图片输出接口接收原图、选区和标注，输出模块负责合成顺序；
- 布局函数和命中测试必须使用同一组尺寸常量；
- 只有捕获入口失败转换成稳定的 `RSH-CAP-xxx` 诊断码；窗口、Surface、剪贴板和 OCR 错误当前转换成面向用户的详细文本，但不写入持久诊断日志；
...
| 调整多贴图能力 | 会话所有权、窗口集合、热键规则、内存策略 | 多窗口拖动、独立关闭、截图重入、资源释放 |
...
- 待命时释放活动会话的 RGBA 图像、Window、Surface、窗口快照和标注元素；事件循环、托盘、热键、WinRT、配置派生信息和空容器仍存在，部分 `Vec`、`String`、`HashMap` 容量可以被后续会话复用；
- 原始截图约占 `4 × W × H` 字节，softbuffer 显示缓冲约再占 `4 × W × H` 字节；
- 3840×2160 时，RGBA 原图约占 31.6MiB，两块基础像素缓冲合计约 63.3MiB；这只是基础估算，不包含捕获后端、窗口系统、分配器和临时输出；
...
- 每张贴图存在期间会长期保留自己的最终 RGBA 图和 Surface，最多八组；八张全屏 4K 贴图的基础像素估算可接近 506MiB，因此上限只保证占用有界，不代表始终低占用；
- 画笔按指针移动事件追加点，目前没有抽稀或点数上限；
- 每次重绘都由 CPU 转换像素并逐像素处理选区外变暗效果；
- 关闭截图会话必须释放会话图像、遮罩窗口和窗口快照，并删除标注元素，但不得清空独立贴图；外层容器可能保留容量，关闭单张贴图只释放该贴图资源；
- 释放资源后 OS 工作集不保证立即精确回到启动值；判断内存泄漏应看活动资源是否归零，以及固定操作循环中工作集是否持续单调增长。
...
| 鼠标位置、捕获显示器、遮罩显示器匹配或截图获取失败 | 简短错误提示并回到待命 | 热键、托盘和贴图继续工作；不得保留旧图、选区、标注、Window 或 Surface | 每次尝试先清旧会话；按 `RSH-CAP-001`～`004` 记录固定诊断；同一次尝试只提示一次 | 已实现 |
...
| 零面积拖框 | 纯水平或纯垂直移动超过 4px 后松手 | 丢弃退化框并继续选择，立即恢复自动锁窗 | `Mode` 仍为 `Selecting`，不出现 COPY/OCR/PIN 的无效选区错误 |
| 连续捕获失败 | 鼠标、显示器或捕获后端连续失败 | 每次明确热键尝试得到一次短提示，随后仍可重试 | 无残留 Window、Surface、旧图或编辑状态；热键 ID 和贴图集合不变 |
...
当前 Release 构建没有控制台，启动失败通过弹窗展示；Debug 构建打印配置路径。捕获入口失败是当前唯一的持久诊断事件：启用 `diagnostics` 时追加到配置文件同目录的 `capture-errors.log`。

- 每行只记录 Unix 秒、固定事件名 `capture_failed` 和 `RSH-CAP-xxx` 错误码；
- 不记录底层错误文本、鼠标坐标、显示器名称或 ID、窗口标题、截图像素、OCR 文字和剪贴板内容；
- 日志最大 64 KiB，达到上限后停止追加；用户删除文件后重新开始记录；
- `diagnostics: false` 完全关闭该持久日志；日志创建或写入失败不得影响会话清理和错误提示；
- 后续若扩展剪贴板、OCR 或窗口日志，必须继续使用稳定代码、固定允许字段和独立大小策略。

## 9. 验收案例

### 9.1 自动测试

下表是回归覆盖契约。校准本文档时，当前 `main` 的 `cargo test` 共 44 项；具体执行结果仍以对应提交的测试输出为准。
...
| 选择状态转换 | 水平零高、垂直零宽拖框保持选择态并恢复自动锁定；反向 1px 正面积拖框进入编辑态 |
...
| 捕获入口失败 | 四种稳定错误码、旧配置兼容、连续失败清理、同一尝试提示去重、日志字段白名单和 64 KiB 上限 |
| 临时 PNG | 同一时刻分配唯一文件、PNG 编码有效、过期筛选、受保护路径、外来文件和 12 小时调度 |

### 9.2 手工端到端测试

下列内容是手工检查清单，不等同于已经执行的验收记录；每次发布需要另行保存受影响场景的结果。

1. 待命时按 `Alt+A`，确认捕获鼠标所在显示器；
2. 移动鼠标经过重叠窗口，确认红框锁定最上层可见窗口；
3. 单击锁定窗口，确认进入编辑态，没有直接复制；
4. 从四个对角方向建立有面积矩形，确认松手后选区正确；
5. 分别做一次纯水平和纯垂直拖动，确认不进入编辑态，并立即恢复光标下窗口的自动锁定；
6. 分别用画笔、直线、矩形和中文文字标注，复制后检查坐标与颜色；
7. 文字输入时键入 `pocr`，确认只产生文字，不触发动作快捷键；
8. 点击 `SELECT`，确认画面内容不变化，旧标注消失；
9. 在画过标注的选区点击 `OCR`，确认识别内容来自原图；
10. 粘贴 OCR 中英文、换行和 Emoji，确认 Unicode 文本完整；
11. 模拟无 OCR 语言包或无文字选区，确认错误后仍可继续编辑；
12. 连续建立八张贴图，确认每张都能独立拖动，并可分别用 `X`、右键和 `Esc` 关闭；
13. 贴图存在时再次按截图热键，确认旧贴图不进入冻结画面；取消、复制或新增贴图后旧贴图全部恢复；
14. 已有八张贴图时尝试置顶第九张，确认提示上限且当前编辑内容不丢失；
15. 在 100%、125%、150% 缩放的多显示器组合下检查选框和输出；
16. 在 4K 显示器上记录启动稳定值、操作峰值和关闭会话后的工作集；固定次数重复截图、OCR、贴图和关闭，确认活动资源归零且工作集没有持续单调增长，无需精确回到启动值；
17. 分别向聊天软件、Word、画图、终端和资源管理器粘贴，检查位图与文件格式。
18. 用辅助程序临时独占剪贴板后执行复制，确认错误提示后原图、选区和标注仍在；释放剪贴板后重试成功。
19. 分别模拟鼠标读取、显示器定位/匹配和屏幕捕获失败，连续触发三次，确认每次只有一条带 `RSH-CAP-xxx` 的提示，Alt-Tab 无残留遮罩；解除故障后立即得到全新截图。

### 9.3 完成定义

以下内容是后续发布的准入规则，不表示仅凭本文档即可证明现有 Release 已保存全部手工与内存验收证据。一次涉及交互或输出的修改满足以下条件后才可发布：

- 状态转移和失败路径已经写清；
- 自动测试通过；
- 受影响的端到端案例通过；
- Release 构建成功；
- 待命、活动截图和关闭后的活动资源符合不变量，固定循环中内存没有持续单调增长；
- README 的操作说明与本文档一致；
...
| D-11 | 按编辑、几何、渲染、剪贴板、OCR、贴图和 Windows 辅助能力拆分；会话所有权及事件编排仍集中于 `App`/`handler` | 限制平台依赖扩散，让部分纯逻辑可独立测试，同时保持当前窗口生命周期集中管理 | 尚未形成独立纯状态机，模块之间仍通过 `pub(super)` 接口和 `App` 编排协作 |
| D-12 | 图片输出以当前遮罩窗口持有剪贴板，至少一种格式发布成功且正常关闭剪贴板才结束会话 | 避免 `OpenClipboard(NULL)` 的无 owner 写入失败，也避免一次剪贴板竞争丢失编辑内容 | 部分成功时会降级为调用方实际收到的单一格式 |
| D-13 | 每次截图热键建立独立捕获尝试，入口失败只记录固定代码并统一清理 | 连续后端故障不能留下旧图或孤儿窗口，错误日志不能泄露桌面信息 | 诊断日志只保留失败代码，无法提供底层后端错误全文 |
