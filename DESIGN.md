...
| 错误提示 | 启动、捕获入口或 OCR 错误 | 启动错误退出；捕获错误回到待命；OCR 错误保留当前编辑会话 |
...
| `src/app/diagnostics.rs` | 捕获失败诊断行、配置目录日志和 64 KiB 大小上限 |
...
当前 Release 构建没有控制台，启动失败通过弹窗展示；Debug 构建打印配置路径。捕获入口失败是当前唯一的持久诊断事件：启用 `diagnostics` 时追加到配置目录旁的 `capture-errors.log`。
...
18. 分别模拟鼠标读取、显示器定位/匹配和屏幕捕获失败，连续触发三次，确认每次只有一条带 `RSH-CAP-xxx` 的提示，Alt-Tab 无残留遮罩；解除故障后立即得到全新截图。
