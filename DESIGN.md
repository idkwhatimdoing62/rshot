...
`App` 只保留热键、托盘、贴图集合和 `Option<CaptureOperation>`。`None` 表示待命；收到截图热键后建立截图操作。截图操作覆盖捕获尝试及成功后的截图会话，内部阶段明确区分 `Preparing`、`Selecting` 和 `Editing`，各阶段只持有当时有效的数据。
...
转为贴图时，`pinned.rs` 新建一张独立贴图窗口。新窗口和 Surface 完全准备成功后，截图操作才结束并把成品 `PinnedWindow` 交给 `App`；创建失败时保留原截图会话、选区和标注。

迁移按可验证行为推进：先迁移开始、捕获失败和取消，再迁移选择、重新选择、编辑和 IME，然后迁移呈现、复制和 OCR，最后迁移贴图创建、失败恢复并删除旧转发代码。每一步保持当前外部行为和测试通过。本次不处理异步 OCR、取消、Windows OCR 独立超时、120ms 轮询、完整贴图集合深化或算法调整。
...
在本设计基线，`cargo test` 共 64 项。下表定义回归覆盖，不用测试数量代替规则覆盖。
