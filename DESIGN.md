...
| R-10 | 已有贴图时开始新截图 | 捕获前暂时隐藏全部贴图，冻结图像素读取完成或失败后立即恢复 | 旧贴图不进入冻结图，恢复时不抢焦点；选择、编辑和 OCR 等待期间贴图保持可见 |
...
| 图片剪贴板 | `CF_DIB` 位图；`CF_HDROP` 指向唯一 PNG | 至少一种格式发布且 `CloseClipboard` 成功后回到待命 | 恢复同一会话；关闭失败视为状态不确定，已经交给 `CF_HDROP` 的文件仍保留 |
...
图片复制成功时，剪贴板发布 module 会把实际发布格式、稳定阶段和事务确定性返回给会话编排；Debug 控制台会打印该结果。Release 没有控制台，也没有成功提示，因此不能把“用户能看到实际格式”当作当前承诺。
...
当前代码由 `App` 持有 `Option<CaptureOperation>` 和 `PinCollection`。截图操作聚合捕获尝试、截图会话、冻结图、窗口资源和交互状态；贴图集合独占 0～8 张贴图的 Window、Surface、最终图像、事件路由和显隐状态。`App` 只协调两个 module 之间的转贴图交接。
...
捕获尝试是 `CaptureOperation::start` 内同步完成的事务。它依次读取鼠标、匹配捕获与遮罩显示器、取得贴图隐藏租约、捕获冻结图、立即完成租约并恢复贴图、读取可见窗口，再创建 Window、Context 和 Surface。只有全部步骤成功才向 `App` 返回正式截图操作；不会把半初始化的 `Preparing` 状态存入应用。
...
冻结图始终是不含标注和旧贴图的屏幕像素。窗口自动锁定、手动框选、重新选择、OCR 和图片输出都引用同一张冻结图。贴图隐藏租约只覆盖桌面像素读取，成功读取或任一失败路径都会无激活恢复全部贴图；截图会话不持有该租约，因此同步 OCR 等待不会让贴图消失。关闭会话后，冻结图、窗口快照、选区和标注一起释放。
...
截图输出是从冻结图、已确定选区和标注生成最终图片的深 module。`src/app/output/` 拥有 `Annotation`、`Shape`、只读输出描述、稳定失败阶段和合成结果；截图会话只提供快照，不把窗口、工具栏、IME 预编辑或交互状态交给输出实现。

输出规则固定如下：

- 选区先归一化并裁切到冻结图；选区决定最终尺寸，选区外标注自然裁切；
- 没有选区时使用整张冻结图；没有选区且没有标注时结果借用冻结图，不复制像素；
- 零面积或裁切后无像素的选区返回 `RSH-OUT-001`，不能退回整屏输出；
- 标注数组顺序就是绘制顺序，IME 预编辑文本、光标、选区框、阴影、工具栏和色板不进入最终图片；
- 文字栅格化失败返回 `RSH-OUT-002`，复制和转贴图均保留当前截图会话，不允许静默输出缺少文字的图片。

标注遍历、坐标原点转换、线宽和绘制顺序只有一份实现，内部连接显示缓冲和 RGBA 图片两个像素 adapter。实时预览使用原点 `(0, 0)`，允许一次文字栅格化失败只跳过当次文字；最终图片使用标准化选区左上角作为原点，并把文字失败提升为完整输出失败。GDI 资源调用仍由 Windows adapter 提供，`render.rs` 只负责实时画面的冻结图铺底、选区 chrome、工具栏、色板和输入提示。

复制直接消费借用或拥有的输出结果。转为置顶贴图时先完成唯一一次截图输出，再创建隐藏贴图窗口；只有两者都成功才提交会话所有权。整屏无标注贴图在提交时直接转移冻结图，其他情况转移已经拥有的合成图片。
...
贴图窗口的宽高取标准化选区的边长，即 `right - left` 和 `bottom - top`；整屏贴图使用冻结图尺寸。窗口尺寸至少为 56×44，为左上角 `PIN` 标识留出空间；当前没有持续施加系统级最小尺寸约束。图像小于窗口时，渲染器以黑色填充空余区域，因此只有小于最小窗口尺寸的贴图可能出现黑色边带。`PIN` 标识属于窗口控件，不属于原图内容。

左键按住并移动贴图，左键双击关闭贴图。双击要求两次短点击间隔不超过 500ms，横纵位置偏差均不超过 4px；单次移动超过该阈值就按拖动处理并清除未完成的双击序列。右键、`Esc` 和系统关闭事件仍可关闭当前贴图。

新贴图先以隐藏状态完整创建 Window、Surface 和最终图像，再插入 `pins` 集合，最后统一执行无激活显示并请求重绘。显示操作必须发生在插入之后，保证第一张贴图立即出现；后续贴图和已有贴图使用同一恢复路径。
...
1. 在打开剪贴板前准备全部可用格式和可转移的 `HGLOBAL`；图片先准备 DIB，同时用受管临时产物准备唯一 PNG；
2. 如果没有任何格式准备成功，不打开也不清空剪贴板；PNG 编码失败会回收半成品，但不阻止 DIB 单独发布；
3. `OpenClipboard` 最多尝试 3 次，重试等待总计约 74ms；`EmptyClipboard`、`SetClipboardData` 和 `CloseClipboard` 不重试；
4. 图片按 `CF_DIB`、`CF_HDROP` 的顺序发布；任一格式成功且正常关闭即为成功；
5. `CF_HDROP` 一旦写入就保留受管文件，包括随后关闭剪贴板失败的状态不确定情形；未写入的本次文件由 Drop 立即回收；
6. 生命周期管理器在启动后首次空闲立即检查一次，之后每 12 小时最多运行一个工作线程；线程启动失败也消耗当前周期；
7. 只删除严格命名、修改时间至少 12 小时、普通且非 reparse point 的文件；单个删除失败只计数并继续；
8. 当前剪贴板仍引用的路径始终保留；无法读取剪贴板时整轮停止；
9. 不递归目录，不处理外来文件；旧版 `%TEMP%\rshot.png` 只按精确路径执行同样规则。
...
12. 截图前一旦隐藏贴图，冻结图像素读取完成或任何失败路径都必须立即、无激活恢复仍存活的贴图；截图会话不得延长隐藏租约；
...
| 独立贴图 | 无边框置顶图和 `PIN` | 拖动当前贴图；双击关闭 | 双击、右键、`Esc` 或系统关闭 |
...
- 捕获失败时由租约释放局部资源并恢复贴图；冻结图成功后立即恢复贴图，因此后续窗口资源创建失败不再影响贴图可见性；
...
**成功结果**：结束截图会话；已有贴图在冻结图捕获完成后已经恢复。已成功发布为 `CF_HDROP` 的 PNG 留给外部程序读取。
...
- 使用与图片复制相同的裁剪和标注合成规则；
- 无选区、无标注时直接转移冻结图所有权；
- 先创建一张隐藏的无边框置顶 Window 和 Surface；
- 请求窗口尺寸为 `max(图像宽,56) × max(图像高,44)`，移动到选区屏幕坐标左上角；
- 最终图成功生成后，用线性 `PreparedPin` 把隐藏窗口、Surface 和最终图原子插入贴图集合；
- 插入完成后显示新贴图、请求重绘并刷新一次桌面合成器；旧贴图在冻结图捕获完成后已经恢复。

**达到上限**：暂时隐藏活动遮罩显示提示，再恢复遮罩；当前截图、标注和八张旧贴图都不变。

**拖动**：贴图窗口收到左键按下时记录全局光标起点和窗口起点；移动时保持初始偏移更新窗口位置；松开时结束拖动。移动超过 4px 会清除未完成的双击序列。该事件只路由到命中的 `WindowId`，不会重新框选或修改贴图像素。

**关闭**：500ms 内、横纵偏差均不超过 4px 的左键双击，以及右键松开、`Esc` 或系统关闭，只移除当前 `WindowId`。单张 resize、buffer 或 present 失败由贴图集合移除故障项，并向 `App` 返回一次 `PinFailure` 通知。

### 3.10 取消、关闭与退出

- 截图窗口最终收到 `Esc`、工具栏 `X` 或系统关闭时，按 Surface、Window 的顺序释放会话资源并清空状态；已有贴图不依赖会话关闭恢复；
- 贴图窗口收到左键双击、`Esc`、右键或系统关闭时，只关闭自身；
...
| 标注数据和定型规则 | 工具栏布局、颜色和新增工具 | 截图输出 interface；选择与编辑规则负责生产和修改 |
| 截图输出 | 显示缓冲、RGBA 像素格式和 GDI 文字栅格化 | 截图输出 module；Windows adapter |
...
当前用 `Option<CaptureOperation>` 把截图会话、冻结图、编辑状态和截图窗口资源收进同一个生命周期边界。同步捕获尝试只有全部成功才产生截图操作，不把半初始化状态存入 `App`。`handler.rs` 中的 `ApplicationHandler<App>` 只按窗口种类路由；属于截图窗口的原始 `WindowEvent` 由截图操作解释，并返回复制、OCR、贴图、关闭或故障等高层命令供 `App` 执行。捕获平台调用位于私有 attempt module；剪贴板发布 module 隐藏 Win32 编码、内存所有权、事务和重试，临时产物 module 独立拥有文件保留与回收；文字识别通过一个 interface 隐藏双后端选择、worker 进程角色、回退与排版实现。
...
| 事件路由 | 热键/托盘轮询；把原始窗口事件先交给贴图集合，未归属事件再交给截图操作 | `src/app/handler.rs` |
| 截图操作与会话状态 | 捕获尝试和截图会话生命周期、阶段、冻结图、选区、编辑状态、Window/Surface 释放顺序 | `src/app/capture_operation/` |
| 捕获与动作编排 | 显示器匹配、桌面捕获、复制、OCR、贴图交接和失败提示 | `src/app/mod.rs`、`src/app/state.rs` |
| 选择与编辑规则 | 模式、工具、标注草稿、几何、工具栏和色板命中；创建和修改输出 module 拥有的标注模型 | `src/app/editor.rs`、`src/app/geometry.rs`、`src/app/state.rs` |
| 截图输出深 module | 标注模型、只读输出描述、选区裁切、坐标转换、共享标注遍历、显示缓冲/RGBA adapter、零拷贝结果和稳定失败阶段 | `src/app/output/` |
| 实时画面与 chrome | 冻结图铺底、选区阴影与边框、工具栏、色板、IME 预编辑和光标提示 | `src/app/render.rs`、`src/app/capture_operation/frame.rs` |
| 贴图集合 | 独占 0～8 张贴图；窗口创建、容量预留、插入后显示、截图显隐、原始事件解释、拖动、双击、重绘故障移除和资源释放 | `src/app/pinned/` |
| 剪贴板发布深 module | 统一接收文字或图片；隐藏 `CF_DIB`、`CF_HDROP`、`CF_UNICODETEXT` 编码、HGLOBAL 所有权、互斥事务、有限重试、实际格式和事务确定性 | `src/app/clipboard/` |
| 受管临时产物生命周期 | 唯一 PNG 创建、默认 Drop 回收、发布后显式保留、12 小时调度、剪贴板引用保护、白名单扫描和逐文件清理报告 | `src/app/temp_artifact.rs` |
...
| 剪贴板发布 | 类型化文字或图片、不透明 owner | 实际发布格式、稳定阶段、事务确定性和诊断 | 准备、打开、清空、写格式或关闭失败 | 无格式时不改变原剪贴板；成功必须有格式且正常关闭 | HWND、CF_*、HGLOBAL 和重试实现 |
| 临时产物生命周期 | 图片创建请求或 `tick(now)` | 受管文件或异步清理报告 | 创建、保护路径读取、线程启动、扫描或删除失败 | 未发布文件自动回收；受保护文件不删除；同一时刻最多一个清理任务 | 文件命名、目录扫描和线程状态 |
...
| 贴图集合 | 最终 RGBA、屏幕位置、窗口尺寸、原始贴图 WindowEvent | 成功接管或事件处理结果 | `PinFailure`：达到上限、窗口资源失败、重复 WindowId 或单张绘制失败 | 创建失败不改集合；提交先插入后显示；其他贴图不变；数量≤8 | HashMap、单张窗口、时钟、全局光标和 DWM 刷新 adapter |
...
| 流程 | 固定步骤 | 可替换部分 |
| --- | --- | --- |
| 开始截图 | 清旧状态 → 定位目标 → 必要时隐藏贴图 → 捕获冻结图 → 立即恢复贴图 → 创建会话 → 选择态 | 捕获库、窗口后端和显示器匹配实现 |
...
| 会话故障 | 检测当前资源 → 只提示一次 → 释放会话 → 待命 | 具体故障来源和提示文本；贴图在捕获阶段已经恢复 |
| 贴图事件 | 集合确认 WindowId → 内部解释原始事件 → 修改、关闭或移除故障项 → 返回固定 outcome | 私有窗口、时钟和光标 adapter |
...
- 新会话状态：进入截图操作的阶段枚举和事件解释，必须保持第 3 节转移和第 2 节不变量。
...
本节记录已落地结构及刻意保留的范围边界。截图操作所有权、显式阶段、状态迁移测试、原始 `WindowEvent` 收口、只借用数据的 `CaptureView`，以及“先完整创建新贴图，再结束旧会话”的交接顺序均已落地。

`App` 只保留热键、托盘、`PinCollection` 和 `Option<CaptureOperation>`。`None` 表示待命；收到截图热键后建立截图操作。截图操作覆盖捕获尝试及成功后的截图会话，内部阶段明确区分 `Preparing`、`Selecting` 和 `Editing`，各阶段只持有当时有效的数据。

`capture_operation` 是对外的深模块。它内部拆分交互状态和窗口资源：

- 私有 `interaction` module 独占选区、指针手势、编辑偏好、标注、修订号和互斥的 Selecting/Editing 阶段；
- `interaction` 直接解释原始 `WindowEvent`，每个事件最多产生一个高层命令、一次重绘请求和一个 IME 光标区域；
- 交互结果不直接调用窗口；截图会话固定先应用 IME 区域，再合并请求一次重绘；
- GDI 文字测量与确定性测试实现位于私有 `TextMetrics` seam 后；
- 窗口资源执行窗口创建、显隐、呈现和释放，返回稳定失败类别及技术详情；
- 外层只确认窗口归属、执行交互结果及 Copy/OCR/Pin/Close 高层命令，并生成用户提示；
- OCR、剪贴板和渲染保留现有模块，截图操作只拥有调用顺序和结果处理；
- 呈现读取轻量画面描述并借用冻结图和标注，不复制整张冻结图。

`pinned/` 是贴图集合 deep module。`PinCollection` 独占内部 HashMap、容量、单张窗口、显隐、事件路由和故障移除；私有 `window` adapter 执行 winit/softbuffer 资源操作，私有 `interaction` module 保存拖动与双击状态。App 只调用准备、提交、原始事件处理、截图前隐藏和截图后恢复这组意图 interface。

转为贴图时，`CaptureOperation` 根据选区边长生成 `PinPlan`，`PinCollection::prepare` 按计划新建隐藏窗口并返回独占借用集合的 `PreparedPin`。窗口和 Surface 准备成功后，截图操作才提交最终 RGBA；`PreparedPin::commit` 先插入集合，再无激活显示全部贴图、请求重绘并统一刷新 DWM。准备令牌提前丢弃会释放隐藏窗口；创建失败时保留原截图会话、选区和标注。

截图操作与贴图集合的所有权深化已经完成。当前保留的范围边界包括异步 OCR、取消、Windows OCR 独立超时、120ms 轮询和截图会话转贴图的跨 module 两阶段协调；后者继续由 `App` 编排，避免贴图集合理解冻结图、选区、标注和 revision。
...
| Q-06 | 用户 | 连续创建大图贴图并再次截图 | 0～8 张贴图 | 内存、贴图集合和新会话 | 第九张拒绝；读取冻结图时隐藏旧贴图，读取完成立即无激活恢复 | 数量≤8；第九张不丢编辑；旧贴图不进入冻结图；OCR 等待时贴图可见 | 贴图管理 / 数量上限和按 ID 隔离 / 当前无持久记录 / 关闭单张释放 |
...
会话关闭先隐藏窗口，再释放 Surface，最后释放 Window 和会话数据。该顺序避免 Surface 继续持有窗口句柄。贴图隐藏只覆盖冻结图像素读取；捕获完成后立即恢复，复制、OCR、取消和置顶不再承担恢复职责。
...
| 状态结构 | `App` 集中协调 | 改造成本低 | 事件和状态耦合，纯规则测试边界有限 | 已替换 |
| 状态结构 | 截图操作所有权边界 + 可测试状态迁移 | 会话资源归属明确，关键转移可独立测试，窗口事件只暴露高层命令 | 截图操作内部仍需协调 winit 事件和窗口资源 | **当前选择** |
...
| D-11 | 按截图操作、编辑、几何、渲染、剪贴板、OCR、贴图和 Windows 边界拆分；`handler` 保留平台适配，截图操作解释自身窗口事件，`App` 执行跨模块动作 | 限制变化扩散，并把单次截图的数据、事件规则和资源归入同一所有权边界 | 平台调用尚未完全收口 |
...
| D-14 | 将捕获尝试和截图会话收拢为截图操作深模块；私有交互 module 独占互斥阶段、编辑状态、revision 和原始窗口事件解释；`App` 只持有可选截图操作并执行高层命令；渲染和输出通过借用式一致快照读取内容 | 集中会话不变量、事件解释、失败恢复和动作顺序，使 interface 成为状态迁移测试表面，并为后续异步 OCR 降低状态组合风险 | 交互 module 直接依赖 winit 事件；窗口烟测仍依赖 Windows/winit |
...
在本设计基线，`cargo test` 共 79 项。下表定义回归覆盖，不用测试数量代替规则覆盖。
...
| 多贴图 | 首次提交先插入后显示、八张容量预留、重复 ID 不替换、创建失败不改集合、隐藏/恢复只刷新一次、事件归属、拖动、双击、右键、Esc、旧事件、重绘故障移除、Drop 释放顺序，以及非零起点选区的计划窗口尺寸等于最终输出尺寸 |
...
12. 第一次置顶后确认贴图立即出现；从非零屏幕坐标选择区域置顶，确认窗口与输出图等宽高且没有额外黑色区域；继续建立八张贴图，确认各自可左键拖动，双击可关闭且拖动不会误触关闭，并可分别用右键和 `Esc` 关闭；
13. 贴图存在时再次截图，确认旧贴图不进入冻结图，并在冻结图读取完成或失败后立即恢复；选择、编辑和 OCR 等待期间保持可见；
...
flowchart TB
    subgraph App["rshot.exe：常驻角色 [Application Container]"]
        ...
        Clip["剪贴板发布<br/>[Component]<br/>clipboard/"]
        Temp["受管临时产物生命周期<br/>[Component]<br/>temp_artifact.rs"]
        ...
        Clip -->|"创建、保留或回收文件格式产物"| Temp
        Runtime -->|"空闲时 tick(now)"| Temp
        Session -->|"识别原始选区"| OCR
...
rshot 的目标是供其他用户长期安装、升级并提交问题，而不是只服务于单一开发者。评价标准因此包含真实 Windows 环境中的可靠性、远程排障能力、升级兼容性和可重复发布，不以“当前机器能够运行”作为完成条件。

当前核心架构方向正确，无需推倒重写；但项目尚未达到可以让陌生用户长期依赖的产品级成熟度。现阶段综合成熟度约为 **6.5/10**：架构和资源所有权已经形成，产品化验证、质量门禁、诊断和升级契约仍不足。

| 维度 | 当前评价 | 主要依据 |
| --- | --- | --- |
| 架构设计 | 7.5/10 | 截图操作、交互、输出、贴图、OCR、剪贴板和临时产物已形成明确边界 |
| 核心可维护性 | 7/10 | 深 module 隐藏了多数资源知识，但 `app/mod.rs` 仍是变化汇聚点，部分平台类型继续泄漏 |
| 自动化测试 | 6.5/10 | 现有九十余项测试覆盖大量状态转换和失败不变量，真实 Win32 组合路径覆盖不足 |
| Windows 运行可靠性 | 6/10 | 关键资源具有恢复路径，多 DPI、剪贴板占用、IME、OCR 制品和升级场景仍主要依赖手工验证 |
| 发布与长期维护 | 5/10 | Release 可构建，但 CI 门禁、统一诊断、升级策略、回退和签名尚未闭环 |

分数是阶段性工程判断，不是版本承诺；架构、测试、发布流程发生实质变化时同步修订依据，不为保持分数而维护分数。

### 8.2 已建立的工程基础

1. `CaptureOperation` 拥有冻结图、交互状态、截图窗口和会话生命周期；原始窗口事件被解释为高层命令后再交给应用编排。
2. 截图输出统一服务复制和贴图，并在无选区、无标注时借用原图，避免无意义复制。
3. `PinCollection` 独占贴图容量、窗口集合、截图显隐租约、事件解释和单张故障回收。
4. OCR interface 隐藏 worker、Windows 回退、失败聚合与结果排版；worker 具有请求上限、超时和父进程异常回收。
5. 剪贴板发布隐藏编码、HGLOBAL 所有权、格式顺序、有限重试和事务确定性；受管临时产物默认回收，只有文件格式发布后显式保留。
6. 关键状态转换和资源不变量已有自动测试，格式检查、全量测试和 Release 构建能够在本地重复执行。

这些基础说明当前复杂度主要来自 Windows 桌面集成本身，现有 module 已经承担了真实的信息隐藏职责。下一阶段应验证这些边界，而不是继续按功能名称拆分更多 module。

### 8.3 主要风险

#### 产品级系统验证不足

现有测试以纯逻辑、状态机和 fake adapter 为主，尚不能单独证明真实 Windows 组合行为。高风险矩阵包括：
...
- 100%、125%、150%、200% DPI；
- 双显示器、负坐标和不同缩放比例；
- OCR worker 缺失、制品损坏、超时及 Windows OCR 回退；
- 资源管理器、浏览器、聊天软件和 Office 等不同剪贴板消费端；
- 旧版本配置和运行时文件直接升级。

#### 自动质量门禁未闭环

全量测试和 Release 构建通过，但严格 Clippy 当前不能作为绿色门禁。公开发布前，GitHub Actions 必须在每次变更上执行并要求全部通过：
...
当前 Clippy 反馈主要集中在交互和渲染代码的嵌套条件、参数团、循环表达和测试布局。它们多数不是现存缺陷，但应在建立门禁前清理，避免以后新增告警淹没真正问题。

#### 顶层编排仍是变化汇聚点

`src/app/mod.rs` 同时包含配置、启动、用户消息、截图/OCR/复制/贴图用例编排、进程角色和大量跨模块测试。它尚未成为失控的 God Object，但已是当前最明显的维护热点。后续只进行一次克制拆分：

- `runtime.rs` 拥有启动、配置、托盘、热键和事件循环准备；
- `app/mod.rs` 保留 `App` 与用例编排；
- 集中的测试迁回实际拥有规则的 module。

#### 平台类型仍有泄漏

截图操作提供的复制和 OCR source 仍暴露 HWND、原始图片引用和坐标；`App` 再把 HWND 包装成剪贴板 owner。长期应让截图操作直接返回不透明 owner，使 `App` 不认识 Win32 窗口句柄。该问题影响边界完整性，不是当前发布阻塞项。

#### 剪贴板与临时产物存在概念环

剪贴板发布创建 `ManagedTempArtifact`，临时产物清理又通过剪贴板读取受保护文件路径。长期目标是向生命周期管理器注入窄的 `ClipboardProtectionReader`，由 `App` 组合两个独立 module。该问题属于中等优先级；在真实需求出现前不引入通用平台抽象。

#### 远程排障信息不足

其他用户提交的“闪退”“没有反应”无法靠本机复现稳定定位。诊断需要扩展到剪贴板、OCR worker、贴图和渲染，并保持隐私边界：

- 使用稳定阶段和错误码；
- 记录程序版本、Windows 版本、DPI、显示器数量和失败阶段；
- 不记录截图像素、OCR 文字、窗口标题或用户文档路径；
- 提供打开日志目录或复制诊断信息的入口；
- 对日志执行固定大小上限和轮转。

#### 升级契约尚未建立

长期安装要求明确配置向后兼容、临时目录与旧文件迁移、OCR 运行时版本隔离、Release 资产命名、语义版本、更新日志和旧版回退。代码签名会直接影响 Windows SmartScreen 信任，应在扩大公开分发前单独决策。

### 8.4 优先行动

#### 公开发布前

1. 清理现有 Clippy 结果并建立 GitHub Actions 强制门禁。完成标准是四条质量命令在干净检出上全部通过。
2. 建立版本化 Windows 回归矩阵，覆盖本节列出的 DPI、多屏、IME、剪贴板、OCR 和升级场景；每次 Release 保存执行结果。
3. 为首次截图、连续截图、贴图共存、OCR 回退和剪贴板消费补充至少一层真实集成或自动化冒烟测试。
4. 将稳定错误码扩展到剪贴板、OCR worker、贴图和渲染，提供隐私安全的诊断导出入口。
5. 写明配置升级、Release 命名、版本号、更新日志和回退策略。
6. 对当前工作树的剪贴板与临时产物重构做提交前代码审查，再提交并发布候选版本。

#### 首批公开用户之后

1. 以实际 issue 的频率和影响排序可靠性工作，不再按架构直觉继续拆 module。
2. 每个确认缺陷先补可复现验证，再修复并纳入回归矩阵。
3. 根据真实变化压力解开剪贴板与临时产物的概念环，并缩小 `app/mod.rs`。
4. 建立一键诊断包、安装/升级体验和代码签名方案。

### 8.5 当前架构策略

暂停继续“深化某个 module”。近期工程投入顺序固定为：

```text
可观测性 → 自动化质量门禁 → Windows 回归测试 → 升级与发布流程
```

当四个方向形成闭环后，再用实际 issue 判断是否需要新的 module 或边界调整。完成产品化阶段的标准是：陌生用户遇到问题时能够提供足以定位的诊断；每个 Release 能在干净环境重复构建并通过固定回归矩阵；升级不会静默破坏配置、临时产物或 OCR 运行时。
