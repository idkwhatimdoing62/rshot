...
rshot 在一个冻结画面上完成选择、编辑、OCR、复制和置顶贴图。程序在待命时只保留托盘、热键和配置，不保留截图像素，适合长期常驻。
...
- 左键在不同主状态下只有一个主要含义：选择态框选、编辑态标注、贴图态移动；
- 图片复制同时支持位图粘贴和文件路径粘贴；
- OCR 只识别原始选区，用户画的标注不会进入识别输入；
- 关闭会话后释放原始截图、标注和窗口对象；
- 同一时间最多存在一个截图或贴图窗口，重复触发不会叠加窗口。
...
| 输入来源 | 内容 | 处理规则 |
| --- | --- | --- |
| 全局热键 | 默认 `Alt+A` 截图、`Alt+D` 退出 | 仅在没有活动窗口时响应截图热键；退出热键结束事件循环 |
...
ScreenshotSession = {
  ...
  mode: Selecting | Editing | Pinned,
  annotations: [Annotation]
}
```

- `session_image` 在选择态和编辑态保存未经标注的原始冻结图，在贴图态保存扁平化后的最终图；
...
进入贴图态前，选区和标注被扁平化为最终 RGBA 图像；整屏且没有标注时直接转移原图，不另建一份同尺寸输出。贴图态不再保留独立标注对象：
...
1. 待命状态没有窗口、绘图表面和截图像素；
2. 活动会话必须同时拥有窗口、绘图表面和会话图像；
3. 编辑态必须有有效选区；
4. 贴图态的选区为空，标注数组为空，`session_image` 保存的是合成后的贴图；
5. `text_editing = true` 时，主状态为编辑态，最后一条标注为文字草稿；
6. `drawing = true` 时，主状态为编辑态，最后一条标注是正在更新的非文字图形；
7. 自动锁定窗口和手动框选都只能引用当前冻结画面的坐标；
8. 重新选择保留原始图像，清除旧选区和全部标注；
9. 图片输出按“裁剪原图 → 按顺序合成标注”生成；
10. OCR 按“裁剪原图 → 必要时缩放 → 系统识别”生成；
11. 任意时刻最多存在一个遮罩或贴图窗口；
12. 关闭活动窗口后，下一次截图必须从新的屏幕捕获和空标注集合开始；
13. 屏幕坐标与当前显示器窗口坐标只通过 `monitor_origin` 换算；
14. 裁剪前必须归一化反向拖拽坐标并限制到图像边界，零宽或零高区域不产生输出；
15. `SetClipboardData` 成功后，全局内存所有权转交 Windows，应用不得再次释放；
16. 当前捕获、编码和 OCR 都在 UI 线程同步推进，没有后台任务并发修改会话。

## 4. 主状态机

`Mode` 只表示活动窗口内部状态。待命状态由 `window = None` 表示，因此判断完整状态时不能只看 `Mode`。
...
| 贴图 `Pinned` | 无边框置顶图和右上角关闭按钮 | 拖动贴图；命中关闭按钮时关闭 | 右键、`Esc`、关闭按钮、退出 |
...
1. 读取鼠标位置；
2. 找到鼠标所在显示器并捕获整屏 RGBA 图像；
3. 重置选区、标注、工具、颜色和输入状态；
4. 保存显示器原点；
5. 在遮罩出现前记录可锁定窗口；
6. 创建全屏无边框遮罩并请求首帧绘制。
...
编码前先隐藏遮罩，避免用户看到卡顿。当前实现无论剪贴板写入是否成功都会关闭会话，这是需要补齐的错误处理缺口。
...
- 窗口移动到选区在屏幕上的左上角；
- 选区和标注被清空，主状态改为贴图态；
- 左键拖动窗口，右键、`Esc` 或右上角 `X` 关闭。

贴图存在时截图热键被忽略；双击托盘仍只显示信息窗口。关闭贴图后恢复截图热键。
...
| `src/app/windows_adapter.rs` | DPI、窗口枚举、光标、GDI 文字、消息框、WinRT 生命周期和托盘图标 |
...
当前只允许一个遮罩或贴图窗口。这个决定简化了窗口所有权、热键重入、临时文件和像素内存管理。代价是贴图存在时不能开始新截图。

重新评估信号：用户需要同时对照多张截图，或贴图期间继续截图成为高频操作。
...
- 贴图存在期间会长期保留最终 RGBA 图和对应 Surface；
- 画笔按指针移动事件追加点，目前没有抽稀或点数上限；
- 每次重绘都由 CPU 转换像素并逐像素处理选区外变暗效果；
- 关闭会话必须清空图像、窗口、标注和窗口快照。
...
| Surface resize、buffer 获取或 present 失败 | 显示一次错误并关闭本次截图 | 常驻进程、热键和托盘继续工作 | 渲染函数返回会话错误；先释放 Buffer，再按 Surface、Window 顺序清理 | 已实现 |
| 图片剪贴板被占用 | 当前实现仍关闭会话 | 原图和标注应保留到用户能重试 | 改为提示并恢复编辑界面 | 设计缺口 |
...
| 场景 | 刺激 | 期望响应 | 判断标准 |
| --- | --- | --- | --- |
| 长期常驻 | 用户数小时不截图 | 不保留截图像素，不创建隐藏遮罩 | `img/window/surface` 为空，托盘和热键仍可用 |
...
| 剪贴板竞争 | 其他程序短暂占用剪贴板 | 用户得到失败提示并能重试 | 会话不应在失败后丢失；整体输出结果尚待返回给调用方 |
| 会话渲染失败 | 创建窗口、resize、buffer 或 present 返回错误 | 只关闭本次截图，提示一次后允许重新截图 | 会话资源为空，热键编号不变；旧窗口事件不影响新会话 |
...
11. 点击 `PIN`，确认左键拖动只移动贴图，右上角 `X` 可关闭；
12. 贴图存在时再次按截图热键，确认不会创建第二个窗口；
13. 在 100%、125%、150% 缩放的多显示器组合下检查选框和输出；
14. 在 4K 显示器上连续执行截图、OCR、贴图和关闭，观察内存是否回落；
15. 分别向聊天软件、Word、画图、终端和资源管理器粘贴，检查位图与文件格式。
...
| D-08 | 同时只允许一个截图或贴图窗口 | 简化状态、热键和资源所有权 | 不能多贴图或贴图时继续截图 |
| D-09 | 使用软件渲染和逐像素合成 | 行为直接、依赖少、容易测试 | 高分辨率复杂交互的性能上限较低 |
| D-10 | 窗口和渲染错误只终止当前会话 | 一次图形后端错误不能带走常驻热键和托盘 | 当前编辑内容会丢失，用户需要重新截图 |
...
图片剪贴板写入需要返回明确结果。打开剪贴板、写 DIB 和写文件格式任一关键步骤失败时，恢复编辑界面并提示重试。成功写入至少一种目标格式后再关闭会话，并告诉调用方实际提供了哪些格式。
