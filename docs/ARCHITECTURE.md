...
- 通过全局热键启动单屏截图操作。
- 自动锁定窗口或手动选择区域，并创建、选择、移动和修改画笔、直线、箭头、矩形、马赛克和文字标注。
...
| 时间输入 | 双击时间窗、剪贴板打开重试、OCR worker 的 20 秒上限、Windows OCR 的 8 秒上限、OCR 操作的 30 秒总截止时间、临时产物保留期 |
...
| 标注集合 | 按创建顺序排列的形状与颜色 | `EditorState` | 可选择并修改；只影响截图输出，不改变冻结图；马赛克先于普通标注合成 |
| 编辑历史 | 最多 100 个提交前/后标注快照及当前选中索引 | `EditorState` | 一次完成的创建、拖动、调整、改色、文字编辑或删除是一项；新编辑清空 Redo |
...
flowchart TD
    ...
    ocr["OcrOperation<br/>会话 ID + 截止时间 + 像素副本"]
...
3. 标注的创建和修改只改变 `EditorState`，不得修改冻结图；选框与控制点只在交互预览中绘制。
4. 马赛克以区域左上角对齐的 12×12 物理像素块从冻结图取平均色；全部马赛克先于普通标注合成。
5. 箭头从拖动起点指向终点，使用 3 px 线宽和固定 12 px、30° 箭头翼；预览和截图输出共用相同几何。
6. 捕获尝试全部成功后才提交 `CaptureSession`。
7. 贴图隐藏租约覆盖完整截图操作，并由截图会话接管；会话结束或析构时恢复贴图。
8. `PinPlan.revision` 必须等于提交时的交互 revision；否则拒绝陈旧提交。
9. 图片复制至少发布一种格式且成功关闭剪贴板后，才可按成功路径结束会话。
10. OCR 只读取冻结图或其选区，不读取标注层。
11. 受管临时产物只有在文件格式成功交给系统后才进入保留状态。
12. 无活动会话和贴图时，不保留截图像素，也不加载 OCR 模型。
13. Undo/Redo 最多保留 100 个已提交编辑；拖动中的中间帧不产生历史项，新编辑清空 Redo。
...
| 多个标注 | 马赛克、画笔、矩形、文字混合 | 马赛克从冻结图稳定取样并先合成，普通标注随后按创建顺序绘制；任一必需文字栅格化失败则整体失败 |
...
捕获顺序固定为：读取光标 → 定位捕获显示器 → 匹配遮罩显示器 → 获取贴图隐藏租约 → 读取冻结图 → 枚举窗口 → 创建 Window、Context 和 Surface → 将隐藏租约随冻结图原子提交给截图会话。截图会话结束时恢复贴图。
...
| Selecting | `Esc`、右键或关闭 | Idle | 关闭会话，不发布输出 |
| Editing | 重新选择 | Selecting | 保留冻结图，清除选区和标注 |
| Editing | `Esc`、右键或关闭 | Idle | 丢弃当前编辑并关闭会话，不发布输出 |
| Editing | 复制成功 | Idle | 发布输出并关闭会话 |
| Editing | OCR | Editing（后台识别） | 隐藏遮罩但保留会话；贴图保持隐藏，事件循环继续响应 |
| Editing（后台识别） | 当前 ID 完成 | Idle | 发布文字并结束会话 |
| Editing（后台识别） | 失败或超时 | Editing | 恢复同一会话供重试 |
| Editing（后台识别） | 再次按截图热键 | Preparing | 取消旧 OCR、关闭旧会话并开始新捕获；随后到达的旧结果丢弃 |
| Editing（后台识别） | 退出热键或关闭 | Idle | 先取消 OCR，再释放截图会话 |
...
`发布图片` 成功后，受管 PNG 的路径经 `CF_HDROP` 交给外部程序。它是一份**本机临时引用**，不是稳定文件地址：

- 下界：文件在被当前剪贴板内容引用期间受保护，且创建后不满 12 小时不会被回收。
- 上界：无保证。回收只发生在清理节律触发时（应用启动后一次，之后每 12 小时一次），且只处理该目录下符合严格文件名白名单的产物；因此到点不一定立刻删除，实测可见存留可超过 16 小时。
- 消费契约：外部程序取得该路径后应**立即复制**到自有位置；不得把路径写入配置、文档或跨会话状态。

#### 异步 OCR 结果协议

`App` 为每次文字识别分配 `OcrSessionId`。`OcrOperation` 拥有 OCR 像素副本、截止时间、结果通道和取消标记；后台实现不能借用截图会话，因此事件线程可以继续处理热键、贴图移动、贴图关闭和重绘。

| 结果 | 产生条件 | 事件线程动作 | 晚到结果规则 |
| --- | --- | --- | --- |
| `Completed` | 当前后台识别返回文字或空文字 | ID 匹配后发布文字或恢复会话 | ID 不匹配时不写剪贴板、不提示、不关闭会话 |
| `Failed` | 模型和 Windows OCR 均失败 | ID 匹配后记录最终 `RSH-OCR-*` 阶段并恢复会话 | ID 不匹配时丢弃 |
| `TimedOut` | 30 秒总截止时间到达且仍无结果 | 标记取消，记录 `RSH-OCR-008`，恢复会话 | 通道接收端关闭；后端结果无副作用 |
| `Cancelled` | 新截图、退出或截图会话关闭 | 关闭接收端并释放旧会话 | 后端结果无副作用 |

模型 worker 自身最多等待 20 秒；模型失败或空结果后，Windows OCR 回退最多等待 8 秒。总截止时间是事件线程的最终保护，不替代两个后端各自的上限。事件线程在 OCR 活动期间最多每 30 毫秒检查一次结果；这是一项响应性实现参数，不是 OCR 完成时延承诺。
...
| 边界 | 截图操作期间创建的新贴图 | 完整截图操作结束前保持隐藏，结束后统一显示 |
...
| `pinned::hide_for_capture` | 当前贴图集合 | 可移交给截图会话的 RAII 可见性租约 | 已隐藏或隐藏失败 | 每个窗口的具体隐藏调用 |
...
剪贴板只在“打开”阶段有限重试；高精度 OCR worker、Windows OCR worker 和整个 OCR 操作分别有等待上限；贴图有数量上限；诊断有文件大小上限。OCR 后端运行在事件线程之外，事件线程按会话 ID 接收结果，因此识别期间仍可处理贴图与窗口事件。任一 OCR worker 超时后都会终止并等待对应子进程，晚到结果不能改变当前会话。
...
本文只列出当前选择。问题背景、备选方案、代价和重新评估条件由 ADR 单独保存，避免一次变更的讨论继续膨胀当前事实说明。

| 当前选择 | 决策记录 |
| --- | --- |
| 一次捕获形成不可变冻结图，资源完整后原子提交截图会话 | [ADR-0001](adr/0001-capture-session-and-frozen-image.md) |
| 复制与贴图共用 `ScreenshotOutput`，图片剪贴板尝试 DIB 和文件两种格式 | [ADR-0002](adr/0002-shared-output-and-clipboard-formats.md) |
| OCR 以会话 ID 在事件线程外执行，旧结果没有副作用 | [ADR-0003](adr/0003-asynchronous-session-scoped-ocr.md) |
| 每张置顶贴图使用独立窗口并按 `WindowId` 隔离 | [ADR-0004](adr/0004-independent-pin-windows.md) |
| 捕获、选择、预览、OCR 和输出统一使用物理像素 | [ADR-0005](adr/0005-physical-pixel-coordinate-system.md) |
| 持久诊断只记录白名单字段和稳定错误码 | [ADR-0006](adr/0006-privacy-safe-diagnostics.md) |
| 截图会话存续期间隐藏全部置顶贴图，并在会话结束时恢复 | [ADR-0007](adr/0007-session-scoped-pin-visibility.md) |
| 马赛克使用固定物理像素块从冻结图取样，并在普通标注前合成 | [ADR-0008](adr/0008-frozen-source-mosaic-annotations.md) |
| 标注对象由 `EditorState` 选择和修改，并使用最多 100 步快照历史 | [ADR-0010](adr/0010-editable-annotations-and-bounded-history.md) |
| Windows OCR 回退运行在可终止的独立 worker 进程中 | [ADR-0011](adr/0011-process-isolated-windows-ocr.md) |

尚未形成新决策的事实限制继续记录在对应功能章节。需要改变上表任一选择时，先在 GitHub Issue 比较方案，再创建替代 ADR；旧 ADR 标记为 Superseded 并链接新记录。
