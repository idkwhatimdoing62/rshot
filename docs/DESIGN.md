...
- 通过全局热键启动单屏截图操作。
- 自动锁定窗口或手动选择区域，并进行画笔、直线、矩形、马赛克和文字标注。
...
5. 捕获尝试全部成功后才提交 `CaptureSession`。
6. 贴图隐藏租约覆盖完整截图操作，并由截图会话接管；会话结束或析构时恢复贴图。
7. `PinPlan.revision` 必须等于提交时的交互 revision；否则拒绝陈旧提交。
8. 图片复制至少发布一种格式且成功关闭剪贴板后，才可按成功路径结束会话。
9. OCR 只读取冻结图或其选区，不读取标注层。
10. 受管临时产物只有在文件格式成功交给系统后才进入保留状态。
11. 无活动会话和贴图时，不保留截图像素，也不加载 OCR 模型。
